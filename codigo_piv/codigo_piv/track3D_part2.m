...
function [objects, cam1toW, cam2toW] = track3D_part2(imgseq1, imgseq2, cam_params)
    ...
    number_of_frames = length(imgseq1);
    objects = [];
...
%         figure();
%         ola(:,:,1)=[uint8(fg_depth1_bin)];
%         ola(:,:,2)=[uint8(fg_depth1_bin)];
%         ola(:,:,3)=[uint8(fg_depth1_bin)];
%         imshow(ola.*im1);
%         
%         figure();
%         ola(:,:,1)=[uint8(fg_depth2_bin)];
%         ola(:,:,2)=[uint8(fg_depth2_bin)];
%         ola(:,:,3)=[uint8(fg_depth2_bin)];
%         imshow(ola.*im2);
